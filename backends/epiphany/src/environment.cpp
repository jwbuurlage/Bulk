...
void environment::spawn(int processors, const char* image_name) {
    if (!is_valid()) {
...
void environment::set_core_syncstate_(int pid, SYNCSTATE state) {
...
void environment::init_application_path_() {
    e_directory_.clear();
    char path[1024];
...
void environment::update_remote_timer_() {
...
void environment::microsleep_(int microseconds) {
...
void environment::ext_malloc_init_() {
...
void* environment::ext_malloc_(uint32_t size) {
    return _malloc(malloc_base_, size);
}

void environment::ext_free_(void* ptr) {
