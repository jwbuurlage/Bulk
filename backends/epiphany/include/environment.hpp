...
class environment : public bulk::environment {
  public:
    ...
    void spawn(int processors, std::function<void(bulk::world&)> f) override final;
#else
    ...
    //template <typename T>
    void spawn(int processors, std::function<void(bulk::world&)> f) {
        // TODO: something that throws a compile-time error
        // only when the function is *used*
        std::cerr << "ERROR: Use bulk-compile-tool to compile lambda kernels.\n";
