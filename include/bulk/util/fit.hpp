 ...
 */
inline std::optional<std::pair<double, double>> fit(
    const std::vector<size_t>& xs, const std::vector<double>& ys) {
  if (xs.size() < 2 || ys.size() != xs.size()) {
  ...
  auto num =
      std::accumulate(points.begin(), points.end(), 0.0, [=](double a, auto p) {
        return a + (p.first - avg_x) * (p.second - avg_y);
      });

  auto denum = std::accumulate(
      xs.begin(), xs.end(), 0.0,
      [=](double a, double x) { return a + (x - avg_x) * (x - avg_x); });
